...
def support_contact_count_30d(tables: Tables) -> pd.Series:
    """Support touchpoints about this order in the 30 days before checkout.

    Point-in-time: only contacts strictly before checkout_ts count. Contacts
    after checkout are mostly about the return itself, so counting them would
    leak the label -- and they don't exist yet when the order is scored.
    """
    ...
    prior = contacts[(lead > pd.Timedelta(0)) & (lead <= pd.Timedelta(days=30))]
    counts = prior.groupby("order_id").size()
    return counts.reindex(tables.orders["order_id"], fill_value=0).astype(float)
