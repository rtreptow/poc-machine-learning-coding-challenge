...
def presale_contact_count(tables: Tables) -> pd.Series:
    """Support contacts on this order placed before checkout -- pre-purchase
    friction (sizing / availability / fit questions)."""
    contacts = tables.support_contacts.merge(
        tables.orders[["order_id", "checkout_ts"]], on="order_id"
    )
    prior = contacts[contacts["contact_ts"] < contacts["checkout_ts"]]
